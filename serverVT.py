    ...
    if sys.argv[1] == "VAE":
        ...
        class SaveModelStrategy(fl.server.strategy.FedAvg):
            def aggregate_fit(
                ...
                weights_agg = super().aggregate_fit(rnd, results, failures)
                if weights_agg is not None:
                    # Save aggregated_weights
                    print(f"Saving round aggregated_metric...")
                    checkpoint = tf.train.Checkpoint(weights_agg = weights_agg)
                    checkpoint.write(rep_file)
                return weights_agg
